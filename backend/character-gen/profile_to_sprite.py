...
Usage:
    python profile_to_sprite_gemini.py --url <image_url> --gender male|female [--output-dir output]
    python profile_to_sprite_gemini.py --image <local_path> --gender male|female [--output-dir output]
...
SPRITESHEET_DIR = "Universal-LPC-Spritesheet-Character-Generator/spritesheets"
...
def generate_sprite_from_features(features, output_dir="generated_sprites"):
    ...
    for animation in ANIMATIONS:
        ...
        result = generate_sprite(
            layers,
            output_filename=f"gemini_character_{gender}_{animation}.png",
    ...
    # Save metadata
    metadata_path = os.path.join(output_dir, "gemini_character_features.json")
...
def main():
    parser = argparse.ArgumentParser(
        description='Generate LPC sprite from profile picture (Google Gemini Vision)',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  python profile_to_sprite_gemini.py --url https://example.com/photo.jpg
  python profile_to_sprite_gemini.py --image photo.jpg --gender female --output-dir my_sprites
  python profile_to_sprite_gemini.py --url https://example.com/photo.jpg --gender male
...
Cost: ~$0.01 per image with GPT-4o
    ...
    if not args.url and not args.image:
        print("✗ Error: Must provide either --url or --image")
        parser.print_help()
        return 1
